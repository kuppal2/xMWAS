...
function(sg,df_matrix,main_text="Integrative network",Xname="X",Yname="Y",Zname=NA,Wname=NA,communitymembership=NA,footer=NA,id_mapping_mat=NA,html.selfcontained = TRUE,max.connections.interactive=100,seednum=100,layoutmatrix=NA){
    ...
    vis.nodes$shape<-node_attr_matrix_sel$shape
    vis.nodes$label<-node_attr_matrix_sel$name
    vis.nodes$label.cex<-node_attr_matrix_sel$label.cex
    vis.nodes$size=node_attr_matrix_sel$vertex.size
    vis.nodes$borderWidth <- 0.5
    #vis.nodes$color.border <- "black"
    
    if(is.na(communitymembership)==TRUE){
        vis.nodes$color <- node_attr_matrix_sel$color
        
    }else{
        vis.nodes$color<-node_attr_matrix_sel$color #sg$palette[communitymembership]
    ...
    visnet<-visIgraph(sg,randomSeed=seednum,layoutMatrix=layoutmatrix) #visNetwork(nodes=vis.nodes,edges=vis.links,main=main_text,width=net.width, height=net.height)
    ...
    visnet<-visPhysics(visnet,stabilization = TRUE)
    visnet<-visEdges(visnet,smooth = TRUE)
