network_orig <-
function (mat, comp = NULL, blocks = c(1, 2), cutoff = NULL,
...
interactive = FALSE, layout.fun = NULL, save = NULL, name.save = NULL)
{
    ...
    if (!any(class.object %in% c(object.pls, object.mixOmics::rcc, object.blocks,
    "matrix")))
    stop(" 'network' is only implemented for the following objects: matrix, pls, plsda, spls, splsda, mixOmics::rcc, sgcca, rgcca, sgccda",
    call. = FALSE)
    if (any(class.object %in% c(object.mixOmics::rcc, object.pls))) {
        p = ncol(mat$X)
        ...
        if (any(class.object %in% object.mixOmics::rcc)) {
    ...
    if (any(class.object %in% object.blocks)) {
        if (is.null(color.node)) {
            color.node = c("#FBB4AE", "#B3CDE3", "#CCEBC5", "#DECBE4",
            "#FED9A6", "#FFFFCC", "#E5D8BD", "#FDDAEC", "#F2F2F2")[1:length(blocks)]
    ...
    call. = FALSE)
    if ((length(color.edge)%%2) != 0 && (class(color.edge) !=
    "function") && isTRUE(symkey))
    ...
    call. = FALSE)
    if (length(lwd.edge) != 1 || !is.finite(lwd.edge) || lwd.edge <=
    0)
    ...
    stop("There is no correlation between these blocks whith these components. Try a different value of 'comp'.",
    call. = FALSE)
    if (is.null(cutoff)) {
        if (interactive) {
            cutoff = 0
    ...
    0, call. = FALSE)
    if (cutoff > max(abs(w)))
    stop("invalid value for 'cutoff'", cutoff, " > ", round(max(abs(w)),
    2), call. = FALSE)
    if (any(class.object %in% object.blocks)) {
        group = NULL
