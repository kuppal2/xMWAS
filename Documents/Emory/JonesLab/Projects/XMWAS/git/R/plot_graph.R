...
function(df,net_node_colors=c("orange", "green","blue","purple"),graphmethod="radial",label.cex=0.3,filename="",net_node_shape=c("rectangle","circle","triangle","star"),maxnodesperclass=100,seednum=100,vertex.size=6,interactive=TRUE){
	...
	nodes_vec_class<-gsub(df$to,pattern="[0-9]*",replacement="")
	...
	for(t1_ind in 1:length(t1)){
        	...
        	dftemp<-dfall[which(nodes_vec_class==t1[t1_ind]),]

		if(is.na(maxnodesperclass)==FALSE){	
		if(nrow(dftemp)>maxnodesperclass){
			dftemp<-dftemp[order(abs(dftemp$weight),decreasing=TRUE),]
			dftemp1<-dftemp[-which(duplicated(dftemp$to)==TRUE),]

			if(nrow(dftemp1)>maxnodesperclass){
				dftemp<-dftemp1[order(abs(dftemp$weight),decreasing=TRUE)[1:maxnodesperclass],]	
			}
				rm(dftemp1)
			}
		}
 ...
 #       e1<-edgeWeights(sg)
	...
	if(is(check_plot,"try-error")){
        ...
        try(plot(sg,layout=layout_with_fr,vertex.size=vertex.size,vertex.label=V(sg)$name,edge.color=edge_colors),silent=TRUE)
